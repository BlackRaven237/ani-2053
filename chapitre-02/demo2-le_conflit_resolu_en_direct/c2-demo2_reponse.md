# Introduction
For this demo, `test` repository would be enough and we are going to make use of both it's : `local` and `remote` branches.

We are told here, to provoke then resolve a merge conflict and showing all steps we followed. 

Our target file for this demo is `main.cpp` and note also, this demo follows the exact steps done in `exercise 6`.
